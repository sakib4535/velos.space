...
import "./markup/gesture-markup.js";
...
let shakaHoldStartedAt = 0;
let shakaLastSeenAt = 0;
...
let fistLastPoint = null;
let fistLastAnchor = null;

let fistLastSeenAt = 0;
let fistOpenFrames = 0;

let directPanPointerId = null;
let directPanLastPoint = null;
...
let wasRightClickGesture = false;

let rightClickStartedAt = 0;
...
let cachedTargetLookup = null;

let cachedTargetLookupAt = 0;
...
function applyCursorAcceleration(
value
) {
const centered =
value - 0.5;
...
/*
 * Always send the hand landmarks to the external markup engine.
 */
window.dispatchEvent(
new CustomEvent(
"velosHandsDetected",
...
/*
 * When markup is active, gesture-markup(4).js owns the hands.
 */
const markupActive =
!!window.velosMarkup
?.isEnabled?.();
...
throw new Error(
`Hand tracking failed. GPU: ${gpuError?.message || gpuError}. CPU: ${cpuError?.message || cpuError}`
);
}
}
...
throw new Error(
"MediaPipe returned no HandLandmarker instance."
);
}
...
/*
 * PUBLIC API USED BY gesture-markup(4).js
 */
window.velosApp = {
currentSlideIndex() {
return (
...
!!cameraStream &&
!!E.video &&
E.video.readyState >= 2,

lastHands:
Array.isArray(lastHands)
...
/*
 * Start preview before waiting for MediaPipe.
 */
startLoop();
 ...
 * ================================================================
 *
 * The markup engine is in:
 *
 * app/markup/gesture-markup(4).js
 *
 * It is imported at the top of this app.js.
 * ================================================================
 */
...
function getTextLinkingSwitch() {
return document
.getElementById(
"markupTextLinkingSwitch"
);
}

function markupEngineReady() {
return !!(
window.velosApp &&
window.velosMarkup &&
typeof window.velosMarkup
.enableMarkup ===
"function" &&
typeof window.velosMarkup
.disableMarkup ===
...
function handTrackingReady() {
return !!window.velosApp
?.isHandTrackingReady?.();
}
...
if (!engineReady) {
button.title =
"Markup engine is still loading";
}
}
...
? "Disable gesture markup"

: handTrackingReady()

? "Enable gesture markup"

: "Enable markup; hand tracking is still starting";
}

if (enableSwitch) {
enableSwitch.disabled =
!engineReady;
...
if (textSwitch) {
const textApiAvailable =
typeof window.velosMarkup
?.setTextLinkingEnabled ===
"function";
...
function toggleMarkupFromUi() {
if (!markupEngineReady()) {
toast(
"Markup engine is not ready yet.",
1600
);
...
console.error(
"Markup toggle failed:",
error
);

toast(
`Markup error: ${error?.message || "unknown error"}`,
2200
);
...
if (ok === false) {
toast(
"Markup could not start. Check drawCanvas and the console.",
2200
);
...
toast(
"AI Markup enabled; waiting for hand tracking",
1800
);
}
...
button.className =
laserButton?.className ||
"control-btn";

button.innerHTML =
'<span class="label-desktop">Markup Loading</span><span class="label-mobile">Loading</span>';
...
section.innerHTML = `
<h3>Gesture Markup</h3>

<div class="settings-item">
<label>
<input
type="checkbox"
id="markupEnabledSwitch"
/>
...
Markup automatically takes control of the hand stream while enabled.
</p>
</div>
...
<input
type="checkbox"
id="markupTextLinkingSwitch"
checked
/>
Link marks to visible slide text
</label>

<p class="hint">
Available when the loaded markup engine supports text linking.
</p>
</div>
...
toast(
"This markup engine does not provide text linking.",
1600
);
...
toast(
"This markup engine does not provide transcript export.",
1800
);
...
if (!transcript) {
toast(
"No markup transcript is available."
);
...
toast(
"Markup transcript exported"
);
}
);
}

function hookClearInk() {
const clearInkButton =
document.getElementById(
"clearInkBtn"
);
...
clearInkButton.addEventListener(
"click",
() => {
window.velosMarkup
?.clearAnnotations?.();
...
console.log(
"[Velos] Markup dashboard connected"
);
...
button.title =
"Markup engine did not load. Check gesture-markup(4).js and the browser console.";
}

console.error(
"[Velos] Markup engine did not become available within 30 seconds."
);
